...
`yandexgpt-lite-0.5.0` получает новость как недоверенный JSON-документ и обязан
вернуть дословные `evidence_quotes`. Код принимает результат только тогда, когда
каждая цитата найдена в реально переданном модели заголовке или тексте, а
значение каждого структурированного факта присутствует в его `source_quote`.
